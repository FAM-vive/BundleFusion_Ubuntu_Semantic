...
__global__ void integrateDepthMapKernel(HashDataStruct hashData, DepthCameraData cameraData) {
	const HashParams& hashParams = c_hashParams;  // 含有相机位姿
	const DepthCameraParams& cameraParams = c_depthCameraParams;

	const HashEntry& entry = hashData.d_hashCompactified[blockIdx.x]; // entry，用来找当前线程处理的voxel所在的Block
	...
	if (screenPos.x < cameraParams.m_imageWidth && screenPos.y < cameraParams.m_imageHeight) {	//on screen
		...
		//if (depth != MINF) {	//valid depth
			...
			if (depth < hashParams.m_maxIntegrationDistance) {// 若该像素的深度值小于最大的重建的距离
				...
				if (abs(sdf) < truncation)
				{
					...
					if (cameraData.d_colorData) {
						curr.color = make_uchar4(color.x, color.y, color.z, 255);  //voxel的颜色
					} else {
						curr.color = make_uchar4(0,255,0,0);
					}

					//初始化体素中和语义相关的部分
					curr.maxSemanticColor = make_uchar4(200, 200, 200, 255);
					if (cameraData.resultMasks_Num && cameraData.d_maskData)
					{
						for (int i = 0; i < 80; i++)
						{
							curr.score[i] = 0;
						...
						for (int i = 0; i < *cameraData.resultMasks_Num; i++)
						{
							// printf("m_imageWidth = %d\n",cameraParams.m_imageWidth);
							// printf("m_imageHeight = %d\n",cameraParams.m_imageHeight);

							unsigned char maskPixelValue = cameraData.d_maskData[cameraParams.m_imageWidth*cameraParams.m_imageHeight*i + screenPos.y*cameraParams.m_imageWidth + screenPos.x];  // mask的像素值
							if (maskPixelValue != 0)
							{ // 当前pixel有语义
								curr.score[maskPixelValue - 1] += 1; // (*cameraData.resultMasks_Num - i);  // 之前是+=1
								// printf("maskPixelValue=%d\n",maskPixelValue);
								curr.isSemanticColored = true;
							}
						}
					}
					// //初始化结束
					...
					//更新maxSemanticColor
					if (curr.isSemanticColored)
					{
						int index = curr.getMaxScoreIndex();
						if (index != -1)
						{
							// printf("curr体素中的index的值是：%d\n", index);
							curr.updateMaxSemanticColor(make_uchar4(cameraData.classColorList_gpu[3 * index], cameraData.classColorList_gpu[3 * index + 1], cameraData.classColorList_gpu[3 * index + 2], 255));
						}
					}
					...
					if (!deIntegrate) {	//integration
						...
						//更新语义
						if (!oldVoxel.isSemanticColored)
						{  // oldVoxel没有语义
							newVoxel.maxSemanticColor = make_uchar4(200, 200, 200, 255);
						}
						else
						{  // oldVoxel有语义
							for (int i = 0; i < 80; i++)
								newVoxel.score[i] = oldVoxel.score[i];
							newVoxel.maxSemanticColor = oldVoxel.maxSemanticColor;
							newVoxel.isSemanticColored = oldVoxel.isSemanticColored;
						}
						...
						{
							if (!oldVoxel.isSemanticColored)
							{  // oldVoxel没有语义,直接把当前语义给newVoxel
								for (int i = 0; i < 80; i++)
								{
									newVoxel.score[i] = curr.score[i];
								}
								newVoxel.maxSemanticColor = curr.maxSemanticColor;
								newVoxel.isSemanticColored = curr.isSemanticColored;
							}
							else
							{  // oldVoxel有语义
								// 用curr体素去更新newVoxel。curr体素的score若大于0，更新newVoxel中的score和maxSemanticColor
								for (int i = 0; i < 80; i++)
								{
									if (curr.score[i] > 0)
										newVoxel.score[i] = newVoxel.score[i] + curr.score[i];  // 更新newVoxel的score
								}
								// 更新newVoxel的maxSemanticColor
								int index = newVoxel.getMaxScoreIndex();
								if (index != -1)
								{
									newVoxel.updateMaxSemanticColor(make_uchar4(cameraData.classColorList_gpu[3 * index], cameraData.classColorList_gpu[3 * index + 1], cameraData.classColorList_gpu[3 * index + 2], 255));
								}
							}
						}
						//更新语义结束
					...
					{//deintegration
						...
						if (newVoxel.weight > 0.001f)
						{  // newVoxel非空
							// newVoxel的score和maxSemanticColor都置为oldVoxel的
							if (oldVoxel.isSemanticColored)
							{
								for (int i = 0; i < 80; i++)
								{
							...
							// newVoxel和curr都有语义时才更新
							//if (newVoxel.isSemanticColored && curr.isSemanticColored){
							if (curr.isSemanticColored)
							{
								// curr体素的score若大于0，更新newVoxel中的score和maxSemanticColor
								for (int i = 0; i < 80; i++)
								{
									if (curr.score[i] > 0)
										newVoxel.score[i] = newVoxel.score[i] - curr.score[i]; // curr.score[i];  // 更新newVoxel的score
									if (newVoxel.score[i] < 0)
										newVoxel.score[i] = 0;
								}
								// 更新newVoxel的maxSemanticColor
								int index = newVoxel.getMaxScoreIndex();
								...
								{
									newVoxel.updateMaxSemanticColor(make_uchar4(cameraData.classColorList_gpu[3 * index], cameraData.classColorList_gpu[3 * index + 1], cameraData.classColorList_gpu[3 * index + 2], 255));
									newVoxel.isSemanticColored = true;
								}
								else
								{
									newVoxel.updateMaxSemanticColor(make_uchar4(200, 200, 200, 255));
									newVoxel.isSemanticColored = false;
									/*printf("经过DeIntegration后，语义被删掉了\n");*/
								}
							}
						}
					}
