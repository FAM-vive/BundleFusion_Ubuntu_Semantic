...
void gSLICrTools::rgbdSuperPixelSeg(cv::Mat rgb,cv::Mat depth,int* gSLICrMask)
{
	int* finalSPixel = new int[width*height];	
	gSLICrInterface(rgb,finalSPixel);
...
void gSLICrTools::gSLICrInterface(cv::Mat inputrgb, int* gSLICrMask)
{
	...
	memcpy(gSLICrMask, gSLICr_engine->Get_Seg_Res()->GetData(MEMORYDEVICE_CPU), 320*240*sizeof(int));
